...
export function Partners() {
  return (
    <section aria-labelledby="partners-heading" className="border-t border-zinc-200 bg-[#f6f6f3] py-20 text-zinc-950 sm:py-24">
      <Container>
        <div className="mx-auto max-w-2xl text-center">
          <h2 id="partners-heading" className="text-4xl font-medium tracking-[-0.05em] sm:text-5xl">Delivery Partners</h2>
        ...
        <ul className="mt-12 flex flex-col border-y border-zinc-300 bg-white sm:flex-row">
          {partners.map((partner) => (
            <li key={partner.id} className="flex h-32 min-w-0 flex-none items-center justify-center border-b border-zinc-200 last:border-b-0 sm:h-40 sm:flex-1 sm:border-b-0 sm:border-r sm:last:border-r-0">
              {partner.status === "verified" && partner.logo ? (
                ...
                <svg aria-hidden="true" viewBox="0 0 64 64" fill="currentColor" className="h-12 w-12 text-zinc-500"><path d={partner.mark} /></svg>
              )}
            </li>
          ))}
        </ul>
