...
const footerLinkClass = "w-fit text-sm leading-6 text-zinc-300 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#a9bdff]";

export function Footer() {
  return (
    <footer className="border-t border-white/15 bg-[#202124] py-16 text-white sm:py-20 xl:pt-24">
      <Container>
        <div className="flex flex-col gap-8 border-b border-white/20 pb-14 sm:flex-row sm:items-start sm:justify-between lg:pb-20">
          <div>
            ...
            <p className="mt-5 max-w-xs text-base leading-7 text-zinc-300">{company.tagline}</p>
          </div>
        ...
        <nav aria-label="Footer navigation" className="flex flex-wrap gap-x-8 gap-y-12 py-14 sm:gap-y-16 lg:gap-x-12 lg:py-20">
          ...
          <div className="flex w-full flex-col gap-3 min-[420px]:w-[calc(50%-1rem)] lg:w-auto lg:flex-1">
            ...
            <p className="text-sm leading-6 text-zinc-300">{company.location}</p>
          </div>
        </nav>

        <div className="flex flex-col gap-6 border-t border-white/20 pt-7 font-[family-name:var(--font-space-mono)] text-[10px] leading-5 tracking-[0.04em] text-zinc-400 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
